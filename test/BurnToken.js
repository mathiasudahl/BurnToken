...
contract('BurnToken', function(accounts){
    ...
    it('initializes the contract with the correct values', function(){
        return BurnToken.deployed().then(function(instance){
            ...
            assert.equal(name, 'Burn Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol, 'BURN', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard, 'Burn Token v.1.0', 'has the correct standard');
        });
    })
    ...
    it('allocates the initial supply upon deployment', function(){
        return BurnToken.deployed().then(function(instance){
            ...
            assert.equal(totalSupply.toNumber(), 1000000, 'sets the total supply to 1,000,000,000');
    ...
    it('transfers token ownership', function(){
        ...
        }).then(assert.feil).catch(function(error){
    ...
    it('handles delegated token transfers', function(){
        return BurnToken.deployed().then(function(instance){
            ...
            assert.equal(balance.toNumber(), 10, 'adds the amount to the reciveing account')
            return tokenInstance.allowance(fromAccount, spendingAccount);
        }).then(function(allowance){
            assert.equal(allowance, 0, 'deducts the amount form the allowance');
        });
    });
})